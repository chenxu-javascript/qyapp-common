 ...
 *
 *
 * Included modules: searchbar,modals,cards,tabs,smart-select,pull-to-refresh,infinite-scroll,scroll-toolbars,fast-clicks,forms,picker,calendar,modals,modals
 *
...
(function () {
    ...
    window.Framework7 = function (params) {
        ...
        ************   Modals   ************
                      ...
                      '<div class="popover-inner">' +
                        ...
                        '<div class="list-block">' +
                          '<ul>' +
                            ...
                            '{{else}}' +
                            '<li><a href="javascript:;" class="item-link list-button {{#if color}}color-{{color}}{{/if}} {{#if bg}}bg-{{bg}}{{/if}} {{#if bold}}actions-popover-bold{{/if}} {{#if disabled}}disabled{{/if}}">{{text}}</a></li>' +
                            '{{/if}}' +
                            '{{/each}}' +
                          '</ul>' +
                        '</div>' +
        ...
        app.closeModal = function (modal, immediately) {
        ...
        ************   Smart Select   ************
        ...
        app.smartSelectOpen = function (smartSelect, reLayout) {
            ...
            if (openIn === 'picker') {
                    ...
                    app._compiledTemplates.smartSelectToolbar = t7.compile(app.params.smartSelectToolbarTemplate ||
                        '<div class="toolbar {{#if toolbarTheme}}theme-{{toolbarTheme}}{{/if}}">' +
                          '<div class="toolbar-inner">' +
                            ...
                            '<div class="right"><a href="javascript:;" class="link close-picker"><span>{{closeText}}</span></a></div>' +
                        '</div>' +
                      '</div>'
                    );
                }
                ...
                navbarHTML = app._compiledTemplates.smartSelectNavbar({
                    ...
                    leftTemplate: openIn === 'popup' ?
                        (app.params.smartSelectPopupCloseTemplate || (material ? '<div class="left"><a href="javascript:;" class="link close-popup icon-only"><i class="icon icon-back"></i></a></div>' : '<div class="left"><a href="javascript:;" class="link close-popup"><i class="icon icon-back"></i><span>{{closeText}}</span></a></div>')).replace(/{{closeText}}/g, closeText) :
                        (app.params.smartSelectBackTemplate || (material ? '<div class="left"><a href="javascript:;" class="back link icon-only"><i class="icon icon-back"></i></a></div>' : '<div class="left sliding"><a href="javascript:;" class="back link"><i class="icon icon-back"></i><span>{{backText}}</span></a></div>')).replace(/{{backText}}/g, backText)
                });
            ...
            var pickerCls = smartSelect.data('pickercls') || '';
            ...
            var searchbarHTML =   '<form class="searchbar searchbar-init" data-search-list=".smart-select-list-' + id + '" data-search-in=".item-title">' +
                                    ...
                                    '</div>' +
                                    (material ? '' : '<a href="javascript:;" class="searchbar-cancel">' + searchbarCancel + '</a>') +
                                  '</form>' +
            ...
            var pageHTML =
                ...
                '      <div class="hairline-bottom list-block ' + (virtualList ? 'virtual-list' : '') + ' smart-select-list-' + id + ' ' + (formTheme ? 'theme-' + formTheme : '') + '">' +
                '        <ul>' +
            ...
            if (openIn === 'popup') {
                ...
                if (reLayout) {
                    ...
                    picker = app.pickerModal(
                        '<div class="picker-modal '+pickerCls+' smart-select-picker smart-select-picker-' + inputName + '"' + (pickerHeight ? ' style="height:' + pickerHeight + '"' : '') + '>' +
                            toolbarHTML +
            ...
            var defaults = {
                ...
                footerTemplate:
                    '<div class="picker-footer">' +
                        '<a href="javascript:;" class="button close-picker">{{closeText}}</a>' +
                    '</div>',
            ...
            function closeOnHTMLClick(e) {
                ...
                if (p.input && p.input.length > 0) {
                    if (e.target !== p.input[0] &&
                       ...
                       !$(e.target).parents(p.input).length
                     ) {
                      p.close();
                    }
                }
